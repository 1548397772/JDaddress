...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component
{
    ...
    switch (component)
    {
        case 0:
            ...
            _citys = [[_provinces objectAtIndex:row] objectForKey:@"cities"];//设置数据源数组
            [_pickerView selectRow:0 inComponent:1 animated:YES];
            [_pickerView reloadComponent:1];
            
            _areas = [[_citys objectAtIndex:0] objectForKey:@"areas"];  //设置数据源数组
            [_pickerView selectRow:0 inComponent:2 animated:YES];
            [_pickerView reloadComponent:2];
            
            _currentProvince = [[_provinces objectAtIndex:row] objectForKey:@"state"]; //设置具体显示的内容
            _currentCity = [[_citys objectAtIndex:0] objectForKey:@"city"];//设置具体显示的内容
            if ([_areas count] > 0) {
                _currentDistrict = [_areas objectAtIndex:0];  //设置具体显示的内容
            } else{
                _currentDistrict = @"";  //设置具体显示的内容
            }
            break;
